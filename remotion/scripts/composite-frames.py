#!/usr/bin/env python3
"""
Composite album art INTO each B-shape intro frame.

Layer order:
  1. Red background (exterior)
  2. Photo/art (visible through B interior — STATIC, full size)
  3. White stroke border on top

Pixel classification:
  - TRANSPARENT + exterior (reachable from edges) → RED
  - TRANSPARENT + interior (NOT reachable from edges) → ART
  - OPAQUE + DARK (R+G+B < brightness threshold) → ART (black fill area)
  - OPAQUE + BRIGHT (white/grey stroke) → keep original (BORDER)
"""
import sys, os
...
RED = (192, 0, 0, 255)
ALPHA_THRESHOLD = 15
BRIGHTNESS_THRESHOLD = 120  # R+G+B sum: below = dark fill, above = stroke
...
def composite_frame(frame_path, art_img, output_path):
    ...
    if art_ratio > frame_ratio:
        # Art is wider -- scale to height, crop sides
        new_h = h
        ...
        # Art is taller -- scale to width, crop top/bottom (favor upper third for faces)
        new_w = w
    ...
    for i in range(w * h):
        ...
        if a >= ALPHA_THRESHOLD and brightness >= BRIGHTNESS_THRESHOLD:
            # BRIGHT OPAQUE = stroke border → keep original
            result_pixels.append(frame_pixels[i])
        elif a < ALPHA_THRESHOLD and exterior[i]:
            # TRANSPARENT + EXTERIOR → red background
            result_pixels.append(RED)
        else:
            # Everything else (dark opaque fill + transparent interior) → art
...
def main():
    ...
    if not art_url_or_path:
        print("Usage: python3 composite-frames.py <art_image_path_or_url>")
        sys.exit(1)

    if art_url_or_path.startswith('http'):
        import urllib.request
        tmp_art = '/tmp/btd-art.jpg'
        urllib.request.urlretrieve(art_url_or_path, tmp_art)
        art_img = Image.open(tmp_art).convert('RGBA')
    else:
        art_img = Image.open(art_url_or_path).convert('RGBA')

    print(f"Art loaded: {art_img.size}")
    ...
    print(f'\n✅ Done! {total + 1} composite frames saved.')
