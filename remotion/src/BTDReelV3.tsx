...
export const BTDReelV3: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  // Toggle: true = video clip, false = photo reveal
  // 'video' | 'reveal' | 'parallax'
  const visualMode = 'reveal' as 'video' | 'reveal' | 'parallax';
  ...
  const artistTier = 1; // Kate Sykes: 73 monthly listeners = Fresh Find
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: bgColor, fontFamily: "'Inter', sans-serif", overflow: 'hidden' }}>
      ...
      <AbsoluteFill style={{ opacity: b2Opacity, zIndex: 10, padding: '0 60px', justifyContent: 'center' }}>
        {/* Country outline — subtle watermark behind name */}
        <div style={{
          position: 'absolute',
          right: -40, top: '50%',
          transform: `translateY(-50%) scale(${interpolate(frame, [68, 100], [0.9, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })})`,
          opacity: interpolate(frame, [68, 100], [0, 0.06], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }),
          width: 700, height: 600,
          pointerEvents: 'none',
        }}>
          <Img src={staticFile('tennessee-outline.svg')} style={{
        ...
        <div>
          ...
          {/* "KATE" — flush left, massive */}
          <div style={{
            fontSize: 220, fontWeight: 900, color: COLORS.text,
            letterSpacing: -4, lineHeight: 0.88, textTransform: 'uppercase',
            marginLeft: 0,
          ...
          <div style={{
            ...
            letterSpacing: -4, lineHeight: 0.88, textTransform: 'uppercase',
            marginLeft: 0,
            opacity: interpolate(frame, [76, 94], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }),
            transform: `translateX(${interpolate(frame, [76, 98], [-80, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })}px)`,
          }}>
            SYKES
          </div>
          {/* Location — readable */}
        ...
        <AbsoluteFill style={{ opacity: b3Opacity, zIndex: 10 }}>
          ...
          {visualMode === 'video' && (
            <div style={{
              ...
              <Video src={staticFile('kate-sykes-pills-hero.jpg')}
          ...
          {visualMode === 'parallax' && (() => {
            ...
            // Slow drift parallax — background moves slower than foreground
            ...
            return (
              <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', opacity: fadeIn }}>
                {/* Background layer — blurred, moves slowly */}
                <div style={{
                ...
                {/* Foreground layer — sharp, moves with parallax */}
                <div style={{
      ...
      <AbsoluteFill style={{ opacity: b5Opacity, zIndex: 10, padding: '0 70px', justifyContent: 'center' }}>
        <div>
          ...
          {[
            { num: '73', label: 'Monthly Listeners', delay: 517 },
            { num: '14K', label: 'Instagram', delay: 523 },
            { num: '0', label: 'Playlist Adds', delay: 529 },
            { num: '1', label: 'Single', delay: 535 },
          ].map((stat, si) => {
      ...
      {(() => {
        const captions: Array<{ text: string; start: number; end: number }> = [
          ...
          // Beat 3: "Outlaw country meets grunge. She calls it Gruntry Western."
          { text: 'Outlaw country meets grunge.', start: 164, end: 210 },
          { text: 'She calls it', start: 210, end: 245 },
          { text: 'Gruntry Western.', start: 245, end: 313 },
          // Beat 5 (data): "73 monthly listeners. 14 thousand on Instagram. Remember this name."
          { text: '73 monthly listeners.', start: 510, end: 548 },
          { text: '14 thousand on Instagram.', start: 548, end: 590 },
          { text: 'Remember this name.', start: 590, end: 660 },
          // CTA: "Go stream Pills right now."
          { text: 'Go stream', start: 668, end: 690 },
          { text: 'Pills', start: 690, end: 715 },
          { text: 'right now.', start: 715, end: 740 },
        ];
