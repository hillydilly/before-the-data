...
export const BTDReelDonKai: React.FC = () => {
  ...
  const b1Opacity = interpolate(frame, [0, 8, 74, 86], [0, 1, 1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
  const b2Opacity = interpolate(frame, [86, 94, 136, 146], [0, 1, 1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
  const b3Opacity = interpolate(frame, [146, 154, 316, 322], [0, 1, 1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
  ...
  const isDark = (frame >= 146 && frame < 510) || frame >= 753;
  const bgColor = isDark ? COLORS.bgDark : COLORS.bg;

  // Frame markers: visible on warm beats only (1,2,5), hidden on dark + video
  const markerBase = interpolate(frame, [0, 8, 136, 146], [0, 1, 1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: bgColor, fontFamily: "'Inter', sans-serif", overflow: 'hidden' }}>
      ...
      <Sequence from={0}>
        <Audio src={staticFile('dk-mixed-audio.mp3')}
          volume={(f) => {
      ...
      <Sequence from={86}><Audio src={staticFile('sfx/sub-hit.mp3')} volume={0.35} /></Sequence>

      {/* Beat 2: paper texture on "Introducing" reveal */}
      <Sequence from={92}><Audio src={staticFile('sfx/paper-texture.mp3')} volume={0.20} /></Sequence>

      {/* Beat 2→3 transition: sub hit as video appears (killed deep sweep — wind sound doesn't fit) */}
      <Sequence from={148}><Audio src={staticFile('sfx/sub-hit.mp3')} volume={0.20} /></Sequence>
      ...
      <AbsoluteFill style={{ opacity: b1Opacity, zIndex: 10, padding: '0 70px', justifyContent: 'center' }}>
        <div style={{ marginTop: -20 }}>
          ...
          {copy.hookLines.map((line, li) => {
            ...
            return (
              <div key={li} style={{
                fontSize: isBold ? 76 : 60, fontWeight: isBold ? 900 : 500,
      ...
      <AbsoluteFill style={{ opacity: b2Opacity, zIndex: 10, padding: '0 60px', justifyContent: 'center' }}>
        ...
        <div style={{
          position: 'absolute', inset: 0,
          opacity: interpolate(frame, [86, 111], [0, 0.28], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }),
          pointerEvents: 'none',
        }}>
          <Img src={staticFile('don-kai-ig-bg.jpg')} style={{
        ...
        <div>
          ...
          <div style={{
            ...
            opacity: interpolate(frame, [90, 106], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }),
            transform: `translateX(${interpolate(frame, [90, 111], [-60, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })}px)`,
          }}>
            {copy.beat2Kicker}
          </div>
          {/* Artist name — two words, slightly smaller to fit */}
          <div style={{
            fontSize: 118, fontWeight: 900, color: COLORS.text,
            ...
            transform: `translateX(${interpolate(frame, [96, 118], [80, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })}px)`,
          }}>
            DON KAI
          </div>
          ...
          <div style={{
            ...
            opacity: interpolate(frame, [118, 136], [0, 0.8], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }),
            transform: `translateY(${interpolate(frame, [118, 138], [20, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })}px)`,
          }}>
            {artistDescriptor}
          </div>
        </div>
      ...
      {/* ============================================ */}
      <Sequence from={146} durationInFrames={170}>
        <AbsoluteFill style={{ opacity: b3Opacity, zIndex: 10 }}>
          {/* === VIDEO VERSION === */}
          {visualMode === 'video' && (
            <div style={{
              position: 'absolute', inset: 0,
              transform: `scale(${interpolate(sp(frame - 146, 5, fps, 14, 0.8), [0, 1], [1.1, 1])})`,
            }}>
              <Video src={staticFile('don-kai-clip.mp4')}
          ...
          {visualMode === 'reveal' && (
            <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
              <div style={{
                ...
                transform: `scale(${
                  interpolate(frame - 146, [0, 150], [1.12, 1.04], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })
                  + Math.sin((frame - 146) * 0.08) * 0.008
                })`,
              }}>
                <Img src={staticFile('don-kai-ig-bg.jpg')}
                  style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 30%' }} />
              </div>
              <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column' }}>
                {Array.from({ length: 12 }, (_, i) => {
                  const barDelay = 5 + i * 3;
                  const barProgress = interpolate(frame - 146, [barDelay, barDelay + 18], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
              ...
              <div style={{
                position: 'absolute', inset: 0,
                filter: `grayscale(${interpolate(frame - 146, [20, 80], [100, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })}%)`,
                mixBlendMode: 'color',
          ...
          {visualMode === 'parallax' && (() => {
            const localF = frame - 146;
            const bgX = interpolate(localF, [0, 170], [0, -20], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
            const bgY = interpolate(localF, [0, 170], [0, -10], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
            const fgX = interpolate(localF, [0, 170], [0, 15], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
            const fgY = interpolate(localF, [0, 170], [0, 8], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
          ...
          <div style={{
            ...
            opacity: interpolate(frame - 146, [20, 40], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }),
            transform: `translateY(${interpolate(frame - 146, [20, 45], [30, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })}px)`,
          }}>
            <div style={{
      ...
      <AbsoluteFill style={{ opacity: b5Opacity, zIndex: 10, padding: '0 70px', justifyContent: 'center' }}>
        <div>
          ...
          {[
            { num: '27K', label: 'Monthly Listeners', delay: 517 },
            { num: '626', label: 'Instagram', delay: 523 },
            { num: '14K', label: 'TikTok', delay: 529 },
            { num: '44K', label: 'Top Track Streams', delay: 535 },
          ].map((stat, si) => {
            ...
            return (
              <>
                ...
                <div style={{
                  ...
                  <Img src={staticFile('don-kai-choose-me-art.jpg')} style={{
      ...
      {(() => {
        const captions: Array<{ text: string; start: number; end: number }> = [
          // Beat 1: "Most people are sleeping on this. Don't be one of them."
          // beat1 audio: 0.000s → 2.672s (frames 0 → 80)
          { text: "Most people are sleeping on this.", start: 4, end: 52 },
          { text: "Don't be one of them.", start: 52, end: 80 },
          // Beat 2: "Don Kai. Singer, songwriter."
          // beat2 audio: 2.872s → 4.662s (frames 86 → 140)
          { text: 'Don Kai.', start: 90, end: 118 },
          { text: 'Singer, songwriter.', start: 118, end: 140 },
          // Beat 3: JT comparison bio
          // beat3 audio: 4.862s → 11.574s (frames 146 → 347)
          { text: 'Don Kai sounds like what Justin Timberlake', start: 150, end: 222 },
          { text: 'would be if he came up in this era.', start: 222, end: 282 },
          { text: 'Smooth, emotional, and completely his own.', start: 282, end: 344 },
          // Beat 5 (data): "27,000 monthly listeners. Over 44,000 streams..."
          // beat4 audio: 16.833s → 23.174s (frames 505 → 695)
          { text: '27,000 monthly listeners.', start: 510, end: 570 },
          { text: 'Over 44,000 streams on his breakout track.', start: 570, end: 655 },
          { text: "He's just getting started.", start: 655, end: 698 },
          // CTA: "Go stream choose me right now."
          // cta audio: 23.374s → 24.885s (frames 701 → 747)
          { text: "Go stream choose me right now.", start: 702, end: 748 },
        ];
