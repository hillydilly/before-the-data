...
// BTD Reel — All We Hold "How I See It"
...
export const BTDReelAllWeHold: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  // Artist config
  const visualMode = 'video' as 'video' | 'reveal' | 'parallax';
  const artistTier = 2; // Rising 📈
  const artistDescriptor = 'Indie Folk Band';
  const postType = artistTier >= 4 ? 'newmusic' : 'discovery';

  // Hook lines
  const hookLines = ["This is the song", "you didn't know", "you needed."] as string[];
  const hookEmphasis = 2; // "you needed." is the bold/italic line
  ...
  // Frame opacity windows — timing based on actual audio durations
  // beat1: 0–62 frames, beat2: 65–142, beat3: 145–284, song: 320–505, data: 505–668, outro: 758+
  const b1Opacity = interpolate(frame, [0, 8, 56, 68], [0, 1, 1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
  const b2Opacity = interpolate(frame, [68, 76, 134, 146], [0, 1, 1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
  const b3Opacity = interpolate(frame, [146, 154, 278, 286], [0, 1, 1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
  const b4Opacity = interpolate(frame, [316, 322, 493, 505], [0, 1, 1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }); // SONG CARD
  ...
  const markerBase = interpolate(frame, [0, 8, 134, 146], [0, 1, 1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: bgColor, fontFamily: "'Inter', sans-serif", overflow: 'hidden' }}>
      ...
      {/* === MIXED AUDIO === */}
      <Sequence from={0}>
        <Audio src={staticFile('awh-mixed-audio.mp3')}
          volume={(f) => {
      ...
      {/* Sound design SFX */}
      <Sequence from={68}><Audio src={staticFile('sfx/sub-hit.mp3')} volume={0.35} /></Sequence>
      <Sequence from={76}><Audio src={staticFile('sfx/paper-texture.mp3')} volume={0.20} /></Sequence>
      <Sequence from={148}><Audio src={staticFile('sfx/sub-hit.mp3')} volume={0.20} /></Sequence>
      <Sequence from={316}><Audio src={staticFile('sfx/bass-drone.mp3')} volume={0.18} /></Sequence>
      ...
      <AbsoluteFill style={{ opacity: b2Opacity, zIndex: 10, padding: '0 60px', justifyContent: 'center' }}>
        ...
        <div style={{
          position: 'absolute', inset: 0,
          opacity: interpolate(frame, [68, 93], [0, 0.28], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }),
          pointerEvents: 'none',
        }}>
          <Img src={staticFile('all-we-hold-ig-bg.jpg')} style={{
        ...
        <div>
          <div style={{
            ...
            opacity: interpolate(frame, [72, 88], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }),
            transform: `translateX(${interpolate(frame, [72, 93], [-60, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })}px)`,
          }}>
            {copy.beat2Kicker}
          </div>
          {/* Artist name — 3 words, stacked */}
          <div style={{
            fontSize: 100, fontWeight: 900, color: COLORS.text,
            letterSpacing: -2, lineHeight: 0.88, textTransform: 'uppercase',
            opacity: interpolate(frame, [78, 96], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }),
            transform: `translateX(${interpolate(frame, [78, 100], [80, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })}px)`,
          }}>
            ALL WE HOLD
          </div>
          {/* Descriptor + Location */}
          <div style={{
            ...
            opacity: interpolate(frame, [100, 118], [0, 0.8], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }),
            transform: `translateY(${interpolate(frame, [100, 120], [20, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })}px)`,
          }}>
            {artistDescriptor}
          </div>
          <div style={{
            ...
            opacity: interpolate(frame, [106, 124], [0, 0.65], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }),
            transform: `translateY(${interpolate(frame, [106, 126], [20, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })}px)`,
          }}>
            Netherlands
          </div>
        </div>
      ...
      {/* BEAT 4: SONG CARD                            */}
      {/* ============================================ */}
      <Sequence from={316} durationInFrames={189}>
        <AbsoluteFill style={{
            ...
            return (
              <>
                ...
                <div style={{
                  ...
                  <Img src={staticFile('all-we-hold-how-i-see-it-art.jpg')} style={{
      ...
      <AbsoluteFill style={{ opacity: b5Opacity, zIndex: 10, padding: '0 70px', justifyContent: 'center' }}>
        <div>
          ...
          {[
            { num: '116K', label: 'Monthly Listeners', delay: 517 },
            { num: '7.5K', label: 'TikTok Followers', delay: 525 },
            { num: '2.2K', label: 'Instagram', delay: 533 },
          ].map((stat, si) => {
            ...
            return (
              <div key={si} style={{
                ...
                opacity: op, transform: `translateX(${x}px)`, marginBottom: 36,
              }}>
                <span style={{
      ...
      {/* ============================================ */}
      {/* CAPTIONS — synced to actual audio timing     */}
      {/* ============================================ */}
      {(() => {
        // Actual durations from ElevenLabs + processing:
        // beat1: 0s → 2.068s (frames 0→62)
        // gap1: 0.1s
        // beat2: 2.168s → 4.747s (frames 65→142)
        // gap2: 0.1s
        // beat3: 4.847s → 9.470s (frames 145→284)
        // silence: 7.363s
        // beat4: 16.833s → 21.595s (frames 505→648)
        // gap3: 0.1s
        // cta: 21.695s → 23.624s (frames 651→709)
        const captions: Array<{ text: string; start: number; end: number }> = [
          // Beat 1: captions removed — no captions at the start
          // Beat 2: "All We Hold. Indie folk band."
          { text: 'All We Hold.', start: 68, end: 96 },
          { text: 'Indie Folk Band.', start: 96, end: 140 },
          // Beat 3: bio
          { text: 'Lyrics that feel like they were already living', start: 148, end: 208 },
          { text: 'in your chest.', start: 208, end: 240 },
          { text: 'Timeless folk. Modern touch.', start: 240, end: 282 },
          // Beat 4 (data)
          { text: '116,000 monthly listeners.', start: 508, end: 555 },
          { text: '7,500 on TikTok. Growing fast.', start: 555, end: 645 },
          // CTA
          { text: 'Go stream How I See It right now.', start: 651, end: 708 },
        ];
